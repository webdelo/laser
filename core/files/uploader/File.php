...
class File
{
	...
	private function checkFilePermissions ($file)
	{
		if (file_exists($file)) {
			...
			if (!in_array($permissions, $this->permissions)) {
				throw new \Exception ('File '.$file.' don\'t have needed permissions! (Current permissions:'.$permissions.')');
			}
		}
	...
	private function checkDirectoryPermissions ($file)
	{
		...
		if (!in_array($permissions, $this->permissions)) {
			throw new \Exception ('Directory '.$directory.' don\'t have needed permissions! (Current permissions:'.$permissions.')');
		}
		return $this;
	}
