					...
					<div class="propertyDetails editPropertyRelation hide">
						...
						<div class="propertyRelation saveRelation" data-action="/admin/<?=$_REQUEST['controller']?>/ajaxEditPropertyRelation/">
							...
							<select class="measurements saveRelationImportant" name="measureId" data-notJumpStep="true">
								...
								<option value="<?=$measure->id?>" <?=$object->getPropertyValueById($value->id)->measureId==$measure->id?'selected=selected':''?> <?=($object->getPropertyValueById($value->id)->measureId=='') && ($measure->id==11)?'selected=selected':''?> ><?=$measure->getShortName()?></option>
								<? endforeach; ?>
							</select>
							<?endif?>
						</div>
...
<!--	<div class="propertiesParts addingMode notSortable">
		<input
			...
			data-post="&additionalCategories[]=<? $config = $property->getConfig(); echo $config::PROPERTY_MODAFURS_CLOTHES_CATEGORY_ID?>"
		/>
	</div>-->
