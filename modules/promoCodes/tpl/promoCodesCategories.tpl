		...
		<div class="main single">
			<div class="max_width">
				<div class="action_buts">
					<a href="/admin/promoCodes/category/"><img src="/admin/images/buttons/add.png" alt="" /> Создать</a>
				...
				<div class="table_edit">
					...
					<table  id="objects-tbl" data-sortUrlAction="/admin/promoCodes/changeCategoriesPriority/?" width="100%">
						<tr>
						...
						<?foreach ($categories as $category):?>
							<tr id="id<?=$category->id?>" class="dblclick" data-url="/admin/promoCodes/category/<?=$category->id?>" data-id="<?=$category->id?>" data-priority="<?=$category->priority?>">
								<td><input type="checkbox" class="groupElements" /></td>
								<td><?=$category->id?></td>
								<td><p class="alias"><a href="/admin/promoCodes/category/<?=$category->id?>/"><?=$category->alias?></a></p></td>
								...
								<td><a href="/admin/promoCodes/category/<?=$category->id?>/" class="pen"></a></td>
								<td><a class="del pointer button confirm" data-confirm="Remove the category?" data-action="/admin/promoCodes/removeCategory/<?=$category->id?>/" data-callback="postRemoveCategory"></a></td>
							</tr>
						<?endforeach?>
					</table>
					<?endif?>
				...
				<div class="action_edit">
					<form id="groupArray" action="/admin/promoCodes/groupActions/?categoriesAction=1" class="groupArray" method="post" data-callback="reloadPage">
						<table>
							<tr>
							...
							<tr class="groupAction groupRemove">
								<td class="first"></td>
								<td>
									<button class="remove button confirm active" name="removeButton" data-confirm="Удалить объекты?" data-action="/admin/promoCodes/groupRemove/?categoriesAction=1" data-data="input[name*=group]" data-callback="reloadPage">ок</button>
								</td>
							</tr>
						</table>
					</form>
