		...
		<table border="0" cellpadding="0" cellspacing="0" style="border-left: 5px solid #24667B; width: 900px;">
			<tr>
				<td style="padding: 0px 0px 0px 10px; width: 60px;">
					...
					<img border="0" src="<?=$this->setImageHere(DIR.'/images/bg/'.$order->domain.'_logo.png')?>" width="50">
					<? else: ?>
					<img border="0" src="<?=DIR_HTTP?>/images/bg/<?=$order->domain?>_logo.png" height="20" alt="">
					<? endif; ?>
				</td>
				...
				<td class="mainHead" style="text-align: right; font-size: 11px; color: #414141; padding: 0px 0px 0px 10px;">
					<?=date('d ')?><?=$this->getObject('\core\utils\DateMonth')->getMonthFullGenetive(date('m'))?><?=date(' Y H:i:s (T)')?>
					<br/>
					<? if($order->nr): ?>Заказ №: <a href="http://<?=$order->domain?>/cabinet/current/<?=$order->nr?>/"><?=$order->nr?></a><br/><? endif; ?>
