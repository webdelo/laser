...
<table>
	...
	<tr>
		<td>
			...
			<form class="goodEditForm" action="/admin/orders/ajaxEditOrderGoodDetails/" data-post="&objectId=<?=$orderGood->id?>">
						...
						<li>
							<div class="propertyRelation saveRelation" data-action="/admin/<?=$_REQUEST['controller']?>/ajaxEditPropertyRelation/">
								...
								<select class="measurements <?= $value->getMeasuresByCategory()->current()->getShortName()=='&nbsp;' ? 'hide' : ''?>" name="measureId" data-notJumpStep="true" style="width: auto;">
									...
									<option value="<?=$measure->id?>" <?=$orderGood->getPropertyValueById($value->id)->measureId==$measure->id?'selected=selected':''?> ><?=$measure->getShortName()?></option>
									<? endforeach; ?>
								</select>
							</div>
						</li>
