		...
		<div class="main single">
			<div class="max_width">
				<div class="action_buts">
					<a href="/admin/parameters/parameter/"><img src="/admin/images/buttons/add.png" alt="" /> Создать</a>
					<a class="filters pointer"><img src="/admin/images/buttons/search.png" alt="" /> Фильтрация</a>
					<a href="/admin/parameters/categories/"><img src="/admin/images/buttons/folder.png" alt="" /> Категории</a>
				...
				<div class="table_edit">
					...
					<table  id="objects-tbl" data-sortUrlAction="/admin/parameters/changePriority/?" width="100%">
						<tr>
						...
						<?foreach ($objects as $object):?>
							<tr id="id<?=$object->id?>" class="dblclick" data-url="/admin/parameters/parameter/<?= $object->id?>" data-id="<?= $object->id?>" data-priority="<?= $object->priority?>">
								<td><input type="checkbox" class="groupElements" /></td>
								<td><?=$object->id?></td>
								<td><p class="alias"><a href="/admin/parameters/parameter/<?=$object->id?>/"><?=$object->alias?></a></p></td>
								<td><p class="name"><?=$object->name?></p></td>
								<td>
									<? if ($object->getParameterValues()->count()>0): $count=0; foreach( $object->getParameterValues() as $value ): $count++?>
										<?=$value->getValue()?><?if ($object->getParameterValues()->count()>$count):?>,<?endif;?>
									<? endforeach; else:?>
								...
								<td><a href="/admin/parameters/parameter/<?=$object->id?>/" class="pen"></a></td>
								<td><a class="del pointer button confirm" data-confirm="Remove the item?" data-action="/admin/parameters/remove/<?=$object->id?>/" data-callback="postRemoveArticle"></a></td>
							</tr>
						<?endforeach?>
					</table>
					<?endif?>
				...
				<div class="action_edit">
					<form id="groupArray" action="/admin/parameters/groupActions/" class="groupArray" method="post" data-callback="reloadPage">
						<table>
							<tr>
							...
							<tr class="groupAction groupRemove">
								<td class="first"></td>
								<td>
									<button class="remove button confirm active" name="removeButton" data-confirm="Удалить объекты?" data-action="/admin/parameters/groupRemove/" data-data="input[name*=group]" data-callback="reloadPage">ок</button>
								</td>
							</tr>
						</table>
					</form>
