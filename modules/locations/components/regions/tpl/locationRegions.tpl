		...
		<div class="main single">
			<div class="max_width">
				<div class="action_buts">
					<a href="/admin/locationRegions/locationRegion/"><img src="/admin/images/buttons/add.png" alt="" /> Создать</a>
				...
				<div class="table_edit">
					...
					<table  id="objects-tbl" data-sortUrlAction="/admin/locationRegions/changePriority/?" width="100%">
						<tr>
						...
						<?foreach ($objects as $object):?>
							<tr id="id<?=$object->id?>" class="dblclick" data-url="/admin/locationRegions/locationRegion/<?= $object->id?>" data-id="<?= $object->id?>">
								<td><input type="checkbox" class="groupElements" /></td>
								<td><?=$object->id?></td>
								<td><p class="alias"><a href="/admin/locationRegions/locationRegion/<?=$object->id?>/"><?=$object->alias?></a></p></td>
								<td><p class="name"><?=$object->getName()?></p></td>
								<td><a href="/admin/locationRegions/locationRegion/<?=$object->id?>/" class="pen"></a></td>
								<td><a class="del pointer button confirm" data-confirm="Remove the item?" data-action="/admin/locationRegions/remove/<?=$object->id?>/" data-callback="postRemoveArticle"></a></td>
							</tr>
						<?endforeach?>
					</table>
					<?endif?>
				...
				<div class="action_edit">
					<form id="groupArray" action="/admin/locationRegions/groupActions/" class="groupArray" method="post" data-callback="reloadPage">
						<table>
							<tr>
							...
							<tr class="groupAction groupRemove">
								<td class="first"></td>
								<td>
									<button class="remove button confirm active" name="removeButton" data-confirm="Удалить объекты?" data-action="/admin/locationRegions/groupRemove/" data-data="input[name*=group]" data-callback="reloadPage">ок</button>
								</td>
							</tr>
						</table>
					</form>
