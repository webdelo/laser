		...
		<div class="main single">
			<div class="max_width">
				<div class="action_buts">
					<a href="/admin/components/component/"><img src="/admin/images/buttons/add.png" alt="" /> Создать</a>
				...
				<div id="filter-form"  style="<?=(isset($_REQUEST['form_in_use'])?'display:block;':'display:none;')?>">
					<form id="search" action="" method="get">
						...
						<table>
							<tr>
								<td colspan="2" class="right">Статус:</td>
								<td>
				...
				<div class="table_edit">
					...
					<table  id="objects-tbl" data-sortUrlAction="/admin/components/changePriority/?" width="100%">
						<tr>
						...
						<?foreach ($objects as $object):?>
							<tr id="id<?=$object->id?>" class="dblclick" data-url="/admin/components/component/<?= $object->id?>" data-id="<?= $object->id?>" data-priority="<?= $object->priority?>">
								<td><input type="checkbox" class="groupElements" /></td>
								<td><?=$object->id?></td>
								<td><p class="alias"><a href="/admin/components/component/<?=$object->id?>/"><?=$object->alias?></a></p></td>
								...
								<td><a href="/admin/components/component/<?=$object->id?>/" class="pen"></a></td>
								<td><a class="del pointer button confirm" data-confirm="Remove the item?" data-action="/admin/components/remove/<?=$object->id?>/" data-callback="postRemoveArticle"></a></td>
							</tr>
						<?endforeach?>
					</table>
					<?endif?>
				...
				<div class="action_edit">
					<form id="groupArray" action="/admin/components/groupActions/" class="groupArray" method="post" data-callback="reloadPage">
						<table>
							<tr>
							...
							<tr class="groupAction groupRemove">
								<td class="first"></td>
								<td>
									<button class="remove button confirm active" data-confirm="Удалить объекты?" data-action="/admin/components/groupRemove/" data-data="input[name*=group]" data-callback="reloadPage">ок</button>
								</td>
							</tr>
						</table>
					</form>
